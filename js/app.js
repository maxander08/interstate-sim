...
const THEATERS = {
  CONUS: {
    ...
    sub: 'US-50 NETWORK · LIVE SIM', unit: 'MI', agency: 'FHWA', style: 'US',
    warn: 'AK / HI RENDERED AS SCALED INSETS — SPANS NOT TO SCALE',
    insets: {
      AK: { p1: 55, p2: 65, lat0: 60, lon0: -152, k: 0.42, dx: -0.413, dy: -0.288 },
      HI: { p1: 18, p2: 23, lat0: 20.6, lon0: -157.4, k: 1.35, dx: -0.254, dy: -0.294 }
    },
    grat2: [{ lons: [-168, -132, 9], lats: [54, 70, 8] }, { lons: [-160, -155, 5], lats: [19, 22, 3] }],
...
function albers(lon, lat) {
  let n = AN, c = AC, r0 = RHO0, l0 = NETW._lon0, sk = 1, dx = 0, dy = 0;
  const ins = NETW._ins;
  if (ins) {   // US-50 theater: AK/HI routed through scaled insets (Albers-USA wall-map layout)
    let I = null;
    if (lat > 50.5 && lon < -129.2) I = ins.AK;
    else if (lon > -161.3 && lon < -154.2 && lat > 17.8 && lat < 23.6) I = ins.HI;
    if (I) { n = I.AN; c = I.AC; r0 = I.RHO0; l0 = I.lon0; sk = I.k; dx = I.dx; dy = I.dy; }
  }
  const rho = Math.sqrt(Math.max(1e-9, c - 2 * n * Math.sin(lat * Math.PI / 180))) / n;
  const th = n * (lon * Math.PI / 180 - l0);
  return [sk * rho * Math.sin(th) + dx, sk * (r0 - rho * Math.cos(th)) + dy];
}
function albersInv(x, y) {
  const ins = NETW._ins;
  if (ins) {
    for (const I of [ins.AK, ins.HI])
      if (x >= I.wx0 && x <= I.wx1 && y >= I.wy0 && y <= I.wy1) {
        const u = (x - I.dx) / I.k, v = (y - I.dy) / I.k;
        const rho = Math.sqrt(u * u + (I.RHO0 - v) ** 2), th = Math.atan2(u, I.RHO0 - v);
        const lat = Math.asin(clamp((I.AC - (rho * I.AN) ** 2) / (2 * I.AN), -1, 1)) * 180 / Math.PI;
        return [I.lon0d + th / I.AN * 180 / Math.PI, lat];
      }
  }
...
/* fold the AK/HI pack into the CONUS theater (runs once, first build) */
function mergeAkhi(T) {
  if (T._merged || typeof NET_AKHI === 'undefined') return;
  T._merged = true;
  const n0 = NET.NODES.length, r0 = NET.ROUTES.length;
  T.net = {
    NODES: NET.NODES.concat(NET_AKHI.NODES),
    EDGES: NET.EDGES.concat(NET_AKHI.EDGES.map(e => [e[0] + n0, e[1] + n0, e[2] + r0])),
    ROUTES: NET.ROUTES.concat(NET_AKHI.ROUTES),
    STATS: {
      miles: NET.STATS.miles + NET_AKHI.STATS.miles, routes: NET.STATS.routes + NET_AKHI.STATS.routes,
      nodes: NET.STATS.nodes + NET_AKHI.STATS.nodes, edges: NET.STATS.edges + NET_AKHI.STATS.edges
    }
  };
  T.states = T.states.concat(STATES_AKHI);
  if (T.counties && typeof COUNTIES_AKHI !== 'undefined') T.counties = T.counties.concat(COUNTIES_AKHI);
  if (T.art && typeof ARTERIALS_AKHI !== 'undefined') T.art = T.art.concat(ARTERIALS_AKHI);
  if (T.hydro && typeof HYDRO_AKHI !== 'undefined') T.hydro = { L: T.hydro.L.concat(HYDRO_AKHI.L), R: T.hydro.R.concat(HYDRO_AKHI.R) };
  if (typeof CITIES_AKHI !== 'undefined') T.cities = T.cities.concat(CITIES_AKHI);
  if (typeof _cityIdxCache !== 'undefined') delete _cityIdxCache['CONUS'];
}
function buildTheater(key) {
  const T = THEATERS[key]; NETW = T;
  if (key === 'CONUS') mergeAkhi(T);
  T._ins = null;
  if (T.insets) {
    T._ins = {};
    for (const k of ['AK', 'HI']) {
      const J = T.insets[k];
      const p1 = J.p1 * Math.PI / 180, p2 = J.p2 * Math.PI / 180;
      const an = 0.5 * (Math.sin(p1) + Math.sin(p2)), ac = Math.cos(p1) ** 2 + 2 * an * Math.sin(p1);
      const rho0 = Math.sqrt(ac - 2 * an * Math.sin(J.lat0 * Math.PI / 180)) / an;
      T._ins[k] = { AN: an, AC: ac, RHO0: rho0, lon0: J.lon0 * Math.PI / 180, lon0d: J.lon0, k: J.k, dx: J.dx, dy: J.dy };
    }
    NETW = T;   // albers() reads NETW._ins below for corner projection
    for (const [k, reg] of [['AK', [-188, 50.5, -129.2, 72]], ['HI', [-161.3, 17.8, -154.2, 23.6]]]) {
      const I = T._ins[k];
      let x0 = 9e9, y0 = 9e9, x1 = -9e9, y1 = -9e9;
      for (let lo = reg[0]; lo <= reg[2] + 1e-6; lo += 2)
        for (const la of [reg[1], reg[3]]) {
          const p = albers(lo, la);
          if (p[0] < x0) x0 = p[0]; if (p[0] > x1) x1 = p[0];
          if (p[1] < y0) y0 = p[1]; if (p[1] > y1) y1 = p[1];
        }
      I.wx0 = x0 - 0.01; I.wx1 = x1 + 0.01; I.wy0 = y0 - 0.01; I.wy1 = y1 + 0.01;
    }
  }
  const P = T.proj;
  ...
  if (T.grat2) for (const g2 of T.grat2) {   // inset-local graticule (AK / HI)
    const [glo0, glo1, gloS] = g2.lons, [gla0, gla1, glaS] = g2.lats;
    for (let lo = glo0; lo <= glo1; lo += gloS)
      GRAT.push([albers(lo, gla0 - 1), albers(lo, gla1 + 1), Math.abs(lo) + '°W', true]);
    for (let la = gla0; la <= gla1; la += glaS)
      GRAT.push([albers(glo0 - 2, la), albers(glo1 + 2, la), la + '°N', false]);
  }
...
function resize() {
  ...
  cam.zFit = Math.min(VW / (WXMAX - WXMIN), VH / (WYMAX - WYMIN)) * 0.89;
